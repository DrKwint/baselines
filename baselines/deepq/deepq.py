...
from baselines.constraint.deepq.utils import ConstraintStateAugmentedInput, ActionHistoryAugmentedInput
    ...
    def make_obs_ph(name):
        if aug_type == 'constraint_state_product':
            return ConstraintStateAugmentedInput(observation_space,
                                                 constraints,
                                                 name=name)
        if aug_type == 'action_history_product':
            return ActionHistoryAugmentedInput(observation_space, action_space,
                                               action_history_len)
        elif aug_type is not None:
            raise Exception(
                'Constraint augmentation {} not implemented'.format(aug_type))
